...
const TicketList = () => {
  ...
  function sortFilter (filterStatus, data) {
    ...
    let copyData = [...data]
    const filter = (status) => copyData.filter((el) => el.segments[0].stops.length !== status && el.segments[1].stops.length !== status)
  ...
  const spiner = <div className={classes.center}><Spin indicator={antIcon} /></div>
  const tickets = viewContent.map((ticket) => {
    return <Ticket key={key++} ticket={ticket} />
  })
  const content = loading ? spiner : err ? <div className={classes.error}><WarningTwoTone style={{ fontSize: 70 }}/><span>Что-то пошло не так, сервер не отвечает, повторите попытку позже</span></div> : tickets
